...
function user_load(Form, fault, success){
  var o = {
    ...
    cmd: JSON.stringify([{
      sgn: "LOG",
      query: $.sprintf(
        "SELECT dbuser,pswd,lang,menu,email,roll,theme,(SELECT access FROM %s WHERE %s.grp=%s.grp) AS access FROM %s WHERE dbuser=? AND (pswd=? OR ?=?)",
...
function user_save(callBack){
  var o = {
    ...
    cmd: JSON.stringify([{
      sgn: "LOG",
      query: $.sprintf(
        "UPDATE %s SET menu=?,lang=?,roll=?,theme=?,pswd=? WHERE dbuser=?",
        SQL_SYS_USER
      ),
      para: [
        oUser.menu,
